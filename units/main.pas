...
begin
  ...
  begin
    ...
    begin
      ...
      Label4.Caption := 'Earliest begin: ' + DateToStr(locDay.Date) + ' (' + TimeToText(locDay.StartHour,locDay.StartMinute ) + ')';
		end;
    if (getDayOfLatestQuitting(FWeekList) <> nil) then
    begin
      locDay := getDayOfLatestQuitting(FWeekList);
      Label5.Caption := 'Latest Leave: ' + DateToStr(locDay.Date) + ' (' + TimeToText(locDay.EndHour,locDay.EndMinute ) + ')';
		end;
	end;
